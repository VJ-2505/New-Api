...
namespace SuperMarketApi.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult UpdateFB([FromForm] dynamic data, IFormFile image)
        {
            try
            {
                dynamic orderJson = data.product.ToObject<Product>();
                if (orderJson.KOTGroupId == 0)
                {
                    orderJson.KOTGroupId = null;
                }
            ...
            try
            {
                var prod = new
                {
                    ...
                    product = from p in db.Products
                              ...
                              select new { p.Id, Product = p.Name, p.Description, p.BarCode , p.Price, p.TakeawayPrice, p.DeliveryPrice, p.CategoryId, p.TaxGroupId, p.CompanyId, p.UnitId, p.ProductTypeId, p.KOTGroupId, p.ImgUrl, p.ProductCode, p.UPPrice, p.Recomended, p.SortOrder, p.isactive, p.minquantity, p.minblock },
